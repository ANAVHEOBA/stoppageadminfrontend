...
function hasSnapshotEnrichment(market: MarketResponse): boolean {
  return (
    market.stats !== undefined ||
    market.quote_summary !== undefined ||
    market.last_trade_yes_bps !== undefined
  );
}
...
function formatMarketHeaderMeta(market: MarketResponse): string {
  return `Ends ${formatShortDate(market.end_time)}`;
}

function formatMarketDisplayOddsLabel(market: MarketResponse): string | null {
  return (
...
function formatMarketBuyYesPriceLabel(market: MarketResponse): string | null {
  const price = normalizeBpsPrice(
    market.quote_summary?.buy_yes_bps ?? market.current_prices?.yes_bps,
  );

  return price === null ? null : formatCents(price);
}

function formatMarketBuyNoPriceLabel(market: MarketResponse): string | null {
  const price = normalizeBpsPrice(
    market.quote_summary?.buy_no_bps ?? market.current_prices?.no_bps,
  );

  return price === null ? null : formatCents(price);
}

function buildActionOutcomeQuotes(
  market: MarketResponse,
  eventSlug: string,
  marketSlug: string,
) {
  const yesPrice = normalizeBpsPrice(
    market.quote_summary?.buy_yes_bps ?? market.current_prices?.yes_bps,
  );
  const noPrice = normalizeBpsPrice(
    market.quote_summary?.buy_no_bps ?? market.current_prices?.no_bps,
  );

  return market.outcomes.map((label, outcomeIndex) => {
    const price = outcomeIndex === 0 ? yesPrice : outcomeIndex === 1 ? noPrice : null;
...
function mergeMarketSnapshots(
  ...
  const mergedMarkets = existingMarkets.map(existingMarket => {
    ...
    return {
      ...
      last_trade_yes_bps:
        incomingMarket.last_trade_yes_bps ?? existingMarket.last_trade_yes_bps ?? null,
    };
  });
...
function buildShellMarket(market: PublicMarketCardResponse): MarketResponse {
  return {
    ...
    last_trade_yes_bps: market.last_trade_yes_bps ?? null,
  };
}
...
function buildFacts(
  ...
  const marketOdds = formatMarketDisplayOddsLabel(selectedMarket);
  const buyYesPrice = formatMarketBuyYesPriceLabel(selectedMarket);
  const buyNoPrice = formatMarketBuyNoPriceLabel(selectedMarket);
  ...
  return [
    ...
    {
      label: "Market odds",
      value: marketOdds ?? "Unavailable",
    },
    {
      label: "Buy yes",
      value: buyYesPrice ?? "Unavailable",
    },
    {
      label: "Buy no",
      value: buyNoPrice ?? "Unavailable",
    },
    {
      label: "Volume",
...
function buildMarketListItem(
  ...
  const actionQuotes = buildActionOutcomeQuotes(market, eventSlug, market.slug);
  ...
  const primaryMetric = formatMarketDisplayOddsLabel(market) ?? "--";
  const href = buildMarketHref(eventSlug, market.slug);
  const volumeLabel = formatMarketVolumeLabel(market);
  const headerMeta = formatMarketHeaderMeta(market);

  return {
    id: market.id,
    slug: market.slug,
    label,
    meta: volumeLabel ?? "Volume unavailable",
    headerMeta,
    href,
    primaryMetric,
    buyYesBps: market.quote_summary?.buy_yes_bps ?? market.current_prices?.yes_bps ?? null,
    buyNoBps: market.quote_summary?.buy_no_bps ?? market.current_prices?.no_bps ?? null,
...
function buildViewModel(context: EventDetailContext): EventDetailViewModel {
  ...
  return {
    ...
    eventPublicationStatus: eventView.event.publication_status ?? null,
