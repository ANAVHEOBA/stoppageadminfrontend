...
function formatSelectionProbability(market: {
  ...
  return (
    formatProbabilityFromBps(
      market.quote_summary?.buy_yes_bps ?? market.current_prices?.yes_bps ?? market.last_trade_yes_bps,
    ) ?? "--"
  );
}
...
function MatchParticipantBadge(props: { participant: EventMatchParticipantResponse }) {
  const label = () => getMatchParticipantCode(props.participant) ?? getMatchParticipantLabel(props.participant).slice(0, 3).toUpperCase();
          ...
          {value => (
            <>
              <A
                ...
                onClick={() =>
                  rememberPreferredMarket(
                    props.card.event.slug,
                    sportPrimaryMarketSlug(),
                  )
                }
              >
    ...
    <section class="pm-all-markets">
      <section class="pm-all-markets__section">
        <div class="pm-all-markets__head">
          <h1 class="pm-all-markets__title">All markets</h1>
        </div>
